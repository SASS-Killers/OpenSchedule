  ...
  if (booked && bookingResult) {
    return (
      <div style={{ textAlign: "center", padding: "2rem 0" }}>
        ...
        >
          {window.location.origin + bookingResult.cancellationUrl}
        </code>
      </div>
    );
  }
    ...
    <div style={{ display: "grid", gap: "1.25rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <button
          ...
          className="app-btn"
          style={{ padding: "0.35rem 0.7rem", minHeight: 0, fontSize: "0.85rem" }}
        >
          ←
        </button>
      ...
      >
        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
          <div key={d} style={{ fontWeight: 600, color: "var(--app-text-muted)", padding: "0.3rem 0" }}>
            {d}
          </div>
        ))}
        {days.map((d, i) => {
      ...
      {selectedDate && (
        <div>
          ...
          {loading ? (
            ...
            <div
              style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: "0.45rem" }}
            >
              {slots.map((ts) => (
                <button
                  key={ts}
      ...
      {selectedSlot && (
        ...
        >
          <div style={{ fontSize: "0.85rem", fontWeight: 600 }}>{formatTime(selectedSlot)}</div>
          ...
          <button
            ...
            style={{ justifyContent: "center", opacity: !name || !email ? 0.6 : 1 }}
          >
            Confirm Booking
          </button>
        </div>
